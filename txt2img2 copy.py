...
for n in range(1):
    # img = Image.open(file)
    api = webuiapi.WebUIApi(host="127.0.0.1" , port=7860 , sampler="DPM++ 3M SDE Karras" , steps=80)
    # controlnet_units = []
    # ctrl : CtrlnetSetting = current.Ctrl
    # unit1 = webuiapi.ControlNetUnit(input_image=img, model=ctrl.ModelName , module=ctrl.ModuleName , weight=ctrl.Weight, control_mode = 1 )
    # controlnet_units.append(unit1)
    # current.UpdatePrompt()
    # p = base + " <lora:tifalockhart-lora-nochekaiser:1:lbw=OUTD> tifa lockhart, black hair, long hair, (low-tied long hair:1.5), (red eyes:1.5), swept bangs, single sidelock,  arm guards, bare shoulders,  black skirt, black sports bra, black thighhighs, collarbone, crop top, earrings, jewelry, midriff, navel, single earring, skirt, sports bra, suspender skirt, suspenders, tank top, teardrop earrings, thighhighs, (white tank top:1.5), black zettai ryouiki  BREAK { <lora:covering_breasts_v0.1:1> , (covering crotch:1.1), (covering breasts:1.1)  | <lora:skirt_tug_v0.1:1> , (skirt tug:1.1)  }  , { standing , upper body , bottomless  | sitting , wariza | sitting  ,spread legs , __nsp/my/new_panties__ , panties , ass   }, open mouth, { front | from above } , { panicking,  surprised | frown , clenched teeth | scared, cry } , tearing up , tentacle-pit , tentacle-pit,tentacles,  in tentacles pit , (many many oily tentacles:1.2), (mucus:1.2), <lora:tentacles_v0.4:0.8:1:lbw=MIDD> , __nsp/my/angle__",
    p = base + "\n 1girl , solo ,  upperbody ,  <lora:tifalockhart-lora-nochekaiser:1:lbw=OUTD> tifa lockhart, black hair, long hair, (low-tied long hair:1.5), (red eyes:1.5), swept bangs, single sidelock,  arm guards, bare shoulders,  black skirt, black sports bra, black thighhighs, collarbone, crop top, earrings, jewelry, midriff, navel, single earring, skirt, sports bra, suspender skirt, suspenders, tank top, teardrop earrings, thighhighs, (white tank top:1.5), black zettai ryouiki  BREAK\n standing in street , smile , cafe , starbucks",
    # print(prompt)
    # print(current.Prompt)
    ...
    r = api.txt2img(
        ...
        cfg_scale=7,
        width=512,#current.Width,
        height=768,#current.Height,
        prompt= str(p),
